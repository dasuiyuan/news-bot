...
ALI_BAILIAN_DEFAULT_MODEL = geoi_settings.ALI_BAILIAN_MODEL
ALI_BAILIAN_API_KEY = geoi_settings.ALI_BAILIAN_API_KEY
ALI_BAILIAN_API_URL = geoi_settings.ALI_BAILIAN_API_URL
...
class AsyncJDYOpenAIClient(AsyncOpenAI):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
...
class ChatAliBailian(CustomLLM):
    ...
    model: str = Field(default=ALI_BAILIAN_DEFAULT_MODEL, description="The ChatQwen model to use.")
    api_url: str = Field(default=ALI_BAILIAN_API_URL, description="The API url.")
    api_key: str = Field(default=ALI_BAILIAN_API_KEY, description="The API key.")
    ...
    def __init__(
            self,
            model: str = ALI_BAILIAN_DEFAULT_MODEL,
    ...
    def _get_client(self) -> AliBailianClient:
        if not self.reuse_client:
            return AliBailianClient(base_url=self.api_url, api_key=self.api_key)

        if self._client is None:
            self._client = AliBailianClient(base_url=self.api_url, api_key=self.api_key)
        return self._client

    def _get_async_client(self) -> AsyncJDYOpenAIClient:
        if not self.reuse_client:
            return AsyncJDYOpenAIClient(base_url=self.api_url, api_key=self.api_key)

        if self._aclient is None:
            self._aclient = AsyncJDYOpenAIClient(base_url=self.api_url, api_key=self.api_key)
