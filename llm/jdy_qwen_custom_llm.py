...
DEFAULT_MODEL = geoi_settings.QWEN_MODEL
Qwen_API_KEY = geoi_settings.QWEN_API_KEY
Qwen_API_URL = geoi_settings.QWEN_API_URL
...
class ChatQwen(CustomLLM):
    ...
    def _get_client(self) -> JDYOpenAIClient:
        if not self.reuse_client:
            return JDYOpenAIClient(base_url=self.api_url, api_key=self.api_key)

        if self._client is None:
            self._client = JDYOpenAIClient(base_url=self.api_url, api_key=self.api_key)
        return self._client
