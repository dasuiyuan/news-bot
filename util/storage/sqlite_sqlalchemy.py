...
class SQLiteDB:
    ...
    def get_after_time(self, model: Type[Base], timestamp: int, types: list = None) -> List[Base]:
        """
        根据时间戳获取所有记录
        :param model: 数据库模型类
        :param timestamp: 时间戳
        ...
        with self.get_session() as session:
            ...
            return session.query(model).filter(model.create_time >= timestamp, model.type.in_(types)).all()
